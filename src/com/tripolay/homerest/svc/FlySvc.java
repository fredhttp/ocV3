...
public class FlySvc
{
	...
	public List<FlyResp> Search()
	{
		...
		try
		{
			Response resp = target.path("svc").path("fly").request().post(Entity.entity(input.toString(),MediaType.APPLICATION_JSON),Response.class);
			LOGGER.log(Level.INFO,String.valueOf(resp.getStatus()));
			fltlist = resp.readEntity(new GenericType<List<FlyResp>>(){});
			sortFlyList(fltlist);
